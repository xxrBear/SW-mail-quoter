...
def gen_cc(msg: Message, except_addresses: List[str]) -> str:
    ...
    if ',' in to_addrs:
        ...
        to_addrs = [addr for addr in to_addrs if not any(excep in addr for excep in except_addresses)]
        cc.extend(to_addrs)
    else:
    ...
    if cc_addrs:
        if ',' in cc_addrs:
            ...
            cc_addrs = [addr for addr in cc_addrs if not any(excep in addr for excep in except_addresses)]
            cc.extend(cc_addrs)
        else:
